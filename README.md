...
**顺带修正分人的过分割**：分人器常把同一个人拆成多簇 —— 真实数据里一场标称 8 人的会，两两相似度中位数是 0.213（正常不同人的量级），却有 4 对落在 0.88~0.93（同人水平）。声纹层允许一个人认领多簇（额外的簇必须**与主簇本身相似** ≥0.80 才并入，只是各自跟注册者沾边不算），那场会里从认领 1 簇变成 4 簇。这是跨会议声纹库带来的独有能力。
